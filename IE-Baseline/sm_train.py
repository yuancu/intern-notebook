...
schema_path = os.path.join(file_dir, 'generated/schemas_me.json')
char_path = os.path.join(file_dir, 'generated/all_chars_me.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chars', type=str, default=char_path)

    args, _ = parser.parse_known_args()

    train_data = json.load(open(args.train))
    dev_data = json.load(open(args.dev))
    id2predicate, predicate2id = json.load(open(args.schema))
    id2predicate = {int(i): j for i, j in id2predicate.items()}
    id2char, char2id = json.load(open(args.chars))
    ...
    for i in range(EPOCH_NUM):
        ...
        with open(os.path.join(args.model_dir, 's_'+str(i)+'.pkl'), 'wb') as f:
            torch.save(s_m, f)
        with open(os.path.join(args.model_dir, 'po_'+str(i)+'.pkl'), 'wb') as f:
            torch.save(po_m, f)
